...
function theme_678studio_styles() {
    ...
    // Enqueue gallery slider on store detail page
    if (is_page_template('page-store-detail.php')) {
        // GSAP Core（既に footer.php で読み込まれているが、依存関係のために追加）
        wp_enqueue_script('gsap', 
            'https://cdnjs.cloudflare.com/ajax/libs/gsap/3.12.2/gsap.min.js', 
            [], '3.12.2', true);
...
add_filter('wp_die_handler', function($handler) {
    wp_log_error('WordPress die handler called', [
        'handler' => $handler
    ]);
    return $handler;
});
...
function ftp_gallery_upload_page() {
    ...
    if (!extension_loaded('ftp')) {
        echo '<div class="error"><p>FTP extension is not available. Please contact your hosting provider.</p></div>';
        return;
    }
