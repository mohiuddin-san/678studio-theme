...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const sliderElement = document.getElementById('registered-stores-slider');
    ...
    if (slides.length === 0) {
        console.warn('No slides found in registered stores slider');
        return;
    }
    ...
    if (storeCount >= 4) {
        ...
        if (track && cardsList) {
            ...
            // 最後のカードに右ボーダーを追加
            const lastCard = slides[slides.length - 1]?.querySelector('.registered-store-card');
            if (lastCard) {
        ...
        const arrows = document.querySelector('.registered-stores-list__arrows');
        if (arrows) {
    ...
    const slider = new Splide('#registered-stores-slider', sliderConfig);

    // Custom arrow buttons (now outside slider element)
    const prevBtn = document.querySelector('.registered-stores-list__arrow--left');
    const nextBtn = document.querySelector('.registered-stores-list__arrow--right');
    ...
    slider.on('mounted', function() {
        console.log('Registered stores slider mounted successfully');
        ...
        if (track) {
            ...
            track.setAttribute('aria-label', '登録店舗一覧');
        }
    ...
    try {
        ...
        console.error('Error mounting registered stores slider:', error);
