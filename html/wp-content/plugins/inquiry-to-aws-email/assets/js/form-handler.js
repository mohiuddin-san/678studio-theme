jQuery(document).ready(function($) {
    ...
    if (typeof siaes_ajax === 'undefined') {
        console.error('siaes_ajax is undefined. Ensure wp_localize_script is called.');
        return;
    }
    ...
    if ($('#shop-id').length === 0) {
        $('#inquiry-form').append('<input type="hidden" id="shop-id" name="shop-id">');
        console.log('Added hidden shop-id field');
    }

    // Handle shop dropdown change
    $('#shop-dropdown').on('change', function() {
        var shopId = $(this).val();
        $('#shop-id').val(shopId);
    ...
    // Initialize shop-id
    var initialShopId = $('#shop-dropdown').val() || '';
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (response.success) {
                    ...
                    $('#shop-dropdown').val(''); // Reset dropdown
