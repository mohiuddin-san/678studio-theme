  ...
  <div class="studio-search-section__container">
    ...
    <div class="studio-search-section__search">
      <div class="studio-search-section__search-box">
        ...
        <input type="text" placeholder="「埼玉県」「横浜市」「大阪」「うめざきまい」" class="studio-search-section__search-input" id="studio-search-input" value="<?php echo esc_attr($search_query); ?>">
      </div>
    </div>
    ...
    <?php if ($total_pages > 1): ?>
      <div class="studio-search-section__pagination">
        <a href="?studio_page=<?php echo max(1, $current_page - 1); ?><?php echo $search_query ? '&studio_search=' . urlencode($search_query) : ''; ?>" class="pagination-btn pagination-btn--prev <?php echo $current_page == 1 ? 'disabled' : ''; ?>">◀</a>
        <div class="pagination-numbers">
          <?php for ($i = 1; $i <= $total_pages; $i++): ?>
            <a href="?studio_page=<?php echo $i; ?><?php echo $search_query ? '&studio_search=' . urlencode($search_query) : ''; ?>" class="<?php echo $i == $current_page ? 'active' : ''; ?>"><?php echo $i; ?></a>
          <?php endfor; ?>
        </div>
        <a href="?studio_page=<?php echo min($total_pages, $current_page + 1); ?><?php echo $search_query ? '&studio_search=' . urlencode($search_query) : ''; ?>" class="pagination-btn pagination-btn--next <?php echo $current_page == $total_pages ? 'disabled' : ''; ?>">▶</a>
      </div>
    <?php endif; ?>
  </div>
</section>
